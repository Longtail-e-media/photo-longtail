...
if ($menuRec):
    $result .= '<ul class="navbar-nav mb-2 mb-lg-0 text-uppercase">';
    foreach ($menuRec as $menuRow):
        ...
        if (!empty($data)):
            ...
            $linkActive = ($menuRow->linksrc == $splitData ) ? " active" : "";
        ...
        $subclass = ($menusubRec) ? 'nav-item px-3' : ' nav-item ms-3';
        $classLink = !empty($menusubRec) ? 'nav-link' : 'nav-link';
...
if ($FmenuRec):
    $resfooter .= '<ul class="d-flex gap-4 list-unstyled footer_menu text-uppercase flex-wrap justify-content-center">';
