...
class Destination extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'slug', 'title', 'title_brief', 'image', 'gallery', 'brief', 'content', 'status', 'sortorder', 'meta_keywords', 'meta_description', 'added_date', 'pradesh');
	...
	public static function get_destination_bypradesh($pradesh)
	{
		global $db;
		$sql = "SELECT * FROM ".self::$table_name." WHERE status=1 AND pradesh = '$pradesh'  ORDER BY sortorder DESC ";
		return self::find_by_sql($sql);
	}
	...
	public static function find_by_id($id=0){
		...
		public static function find_des_by_id($id=0){
		global $db;
		$result_array = self::find_by_sql("SELECT title FROM ".self::$table_name." WHERE id={$id} LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
	}
