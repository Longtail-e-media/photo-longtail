...
class GalleryImage extends DatabaseObject {
	...
	protected static $db_fields = array('id', 'galleryid', 'title', 'detail', 'status', 'sortorder', 'registered', 'image', 'homepage');
	...
	public static function getTotalImages($id=''){
		...
		public static function getTotalImages_home($id=''){
		global $db;
		$cond = !empty($id)?' AND galleryid='.$id:'';
		$query = "SELECT COUNT(id) AS tot FROM ".self::$table_name." WHERE status=1 AND homepage=1 $cond ";
		$sql = $db->query($query);
		$ret = $db->fetch_array($sql);
		return $ret['tot'];
	}
	...
	//Find all the homepage image in the current database table.

	static function getGalleryImagesHome($id=0, $total=150, $offset=0){
		global $db;
		return self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE status=1 AND homepage='1' ORDER BY sortorder ASC  LIMIT {$total} OFFSET {$offset}");
	}
