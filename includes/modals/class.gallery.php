...
class Gallery extends DatabaseObject
{
    ...
    protected static $db_fields = array('id', 'slug', 'image', 'title', 'status', 'sortorder', 'detail', 'type', 'registered', 'content', 'pradesh', 'destinationId');
    ...
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status='1' AND type='$pid' ORDER BY sortorder DESC ";
        return self::find_by_sql($sql);
    }
    
    public static function getParentgallery_by_destination($desid, $pid = '1')
    {
        global $db;
        $sql = "SELECT * FROM " . self::$table_name . " WHERE status='1' AND type='$pid' AND destinationId='$desid' ORDER BY sortorder DESC ";
        return self::find_by_sql($sql);
    }
