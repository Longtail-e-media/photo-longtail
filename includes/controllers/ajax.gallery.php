<?php 
	...
	{			
		case "add":
			...
			$Gallery->image			= (!empty($_REQUEST['imageArrayname'])) ? $_REQUEST['imageArrayname'] : '';			
			$Gallery->status		= 1;
			$Gallery->sortorder		= Gallery::find_maximum();														
			$Gallery->registered	= registered();	
			
			$Gallery->content	= $_REQUEST['content'];

            $Gallery->destinationId	= $_REQUEST['destinationId'];
            $Gallery->pradesh	= Destination::field_by_id($_REQUEST['destinationId'],'pradesh');
